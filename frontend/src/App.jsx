...
import hackerManGif from './hacker-man-hacker.gif';
...
const NavBar = () => {
  ...
  return (
    <motion.nav
      className="bg-indigo-900/40 backdrop-blur-xl text-white fixed w-full z-20 top-0 shadow-xl"
      {...navVariants}
    >
        ...
        <div className="flex justify-between items-center h-16">
          ...
          <div className="hidden sm:flex sm:items-center space-x-4">
            {navItems.map((item) => (
              <motion.a
                key={item.name}
                href={item.href}
                className="px-4 py-2 text-sm font-medium hover:bg-indigo-700/50 rounded-lg"
          ...
          <div className="sm:hidden flex items-center">
            <motion.button
              ...
              className="p-2 rounded-lg text-gray-200 hover:bg-indigo-700/50"
              whileHover={{ scale: 1.05 }}
              whileTap={{ scale: 0.95 }}
              transition={{ type: 'spring', stiffness: 300 }}
            >
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            className="sm:hidden bg-indigo-900/40 backdrop-blur-xl"
            ...
            <div className="pt-2 pb-3 space-y-1">
              {navItems.map((item) => (
                <motion.a
                  key={item.name}
                  href={item.href}
                  className="block px-4 py-2 text-base font-medium text-white hover:bg-indigo-700/50"
...
const Header = () => {
  const [bgImage, setBgImage] = useState(hackerManGif);

  const handleBgError = (e) => {
    console.error('Failed to load background GIF:', e.target.src);
    setBgImage(fallbackImage);
  };

  const handleFallbackError = (e) => {
    console.error('Failed to load fallback image:', fallbackImage);
    setBgImage('https://via.placeholder.com/150?text=Fallback');
  };
  ...
  return (
    <motion.section
      className="relative text-white pt-28 pb-20 min-h-[70vh] flex items-center"
      {...headerVariants}
    >
      <img
        src={bgImage}
        ...
        onError={(e) => {
          handleBgError(e);
          if (bgImage === fallbackImage) {
            handleFallbackError(e);
          }
        }}
      />
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex flex-col items-center text-center relative z-10">
        <motion.div
          className="relative w-32 h-32 sm:w-40 sm:h-40 md:w-48 md:h-48 rounded-full overflow-hidden border-4 border-white/90 shadow-2xl"
        ...
        <motion.h1
          className="text-3xl sm:text-4xl md:text-5xl font-extrabold mt-6 tracking-tight"
        ...
        <motion.p
          className="text-lg sm:text-xl md:text-2xl mt-3 text-indigo-100/90"
          initial={{ opacity: 0, y: 20 }}
          animate={{ opacity: 1, y: 0 }}
...
const About = () => {
  ...
  return (
    ...
    >
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <motion.h2
          className="text-3xl sm:text-4xl md:text-5xl font-extrabold text-indigo-900 tracking-tight"
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="mt-8 grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
          {skills.map((skill, index) => (
            <motion.div
              key={skill}
              className="bg-indigo-50 p-4 rounded-xl text-center text-indigo-800 font-medium shadow-sm hover:shadow-xl border border-indigo-200/50"
              {...skillVariants}
...
const ProjectCard = ({ title, description, link }) => {
  return (
    <motion.div
      className="bg-white/90 backdrop-blur-md p-6 rounded-2xl shadow-xl border border-indigo-100/30"
  ...
  return (
    ...
    >
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <motion.p
          className="mt-6 text-base sm:text-lg md:text-xl text-slate-600 max-w-3xl mx-auto"
          initial={{ opacity: 0, x: 30 }}
          whileInView={{ opacity: 1, x: 0 }}
          transition={{ duration: 0.5, delay: 0.2 }}
        >
        ...
        <form onSubmit={handleSubmit} className="mt-8 max-w-lg mx-auto space-y-6">
          <motion.div className="mb-4" {...inputVariants}>
            ...
            <motion.input
              ...
              required
              className="mt-1 block w-full border border-slate-300 rounded-lg p-3 focus:ring-indigo-500 focus:border-indigo-500"
              {...inputVariants}
            />
          </motion.div>
          <motion.div className="mb-4" {...inputVariants}>
          ...
          <motion.button
            type="submit"
            className="w-full bg-indigo-600 text-white py-3 px-4 rounded-lg hover:bg-indigo-700 shadow-lg disabled:bg-indigo-400"
...
const App = () => {
  useEffect(() => {
    ...
    const numberOfParticles = Math.min(50, Math.floor(window.innerWidth / 20)); // Reduced for performance
    ...
    function animate(timestamp) {
      if (timestamp - lastFrameTime < 16.67) { // Throttle to ~60fps
  ...
  return (
    <div className="font-sans relative min-h-screen bg-indigo-950">
      <style>
        {`
          body {
